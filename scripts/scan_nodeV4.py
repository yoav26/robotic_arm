...
def scan_node():
    ...
    while not rospy.is_shutdown():
        ...
        if j < len(world_angles_mat[0]):  #print(angles_mat[:, j])
            ...
            #create(current_angles, 'angles_w.csv')  # write to file
            #create(joints_angles_rad, 'all_angles_r.csv')  # write to file
...
def calc_scan_points(angles):
    ...
    d = 2
    plain_offset = -l4
    x7 = np.array(range(-16, 18, d))
    y7 = np.array(range(-16, 18, d))
